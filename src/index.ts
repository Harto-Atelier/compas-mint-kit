...
import { runBenchmarkCli } from "./latency-benchmark";
...
Usage
  ...
  npm run dev -- benchmark --samples 25 --format table
                                                 benchmark warmed Robinhood RPC route latency from this VPS
...
async function main(): Promise<void> {
  ...
  if (rawArgs[0] === "benchmark") {
    try {
      await runBenchmarkCli(rawArgs.slice(1));
      closePrompts();
      process.exit(0);
    } catch (err: any) {
      closePrompts();
